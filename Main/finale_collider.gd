...
@export var end_animation_node : Node2D
...
func _ready() -> void:
	...
	if not parent_area is Area2D:
		push_error("End Colliders's parent must be an Area2D.")
		return
...
func _on_entered(node: Node) -> void:
	...
	if character:
		_is_triggered = true
		character.force_stop = true
		
		end_animation_node.visible = true
		end_animation_node.get_node("AnimationPlayer").play("win")
